...
| ❌ | G_能量预算_对手wood | b_low_cost | b_high_cost | slot1 选 4费爆预算后剩 2 能量给 3slot → 应选 1费克制保节奏 |
...
### E_反向幻觉_玩家近期出water
- reasoning: `玩家候选全light无暗，无法凑2:2；预判玩家出fire，我出水克fire×1.5=8伤，留fire/wood覆盖后续。`
...
- 期望: `b_test_wood`
- 实际: `b_test_fire`
  - Run 1 reasoning: `玩家全水牌，预判我出木克水；我偏出火同色打乱节奏，留木给后续slot稳覆盖。`

### ❌ G_能量预算_对手water
- 场景: slot1 选 4费爆预算后剩 2 能量给 3slot → 应选 1费克制保节奏
- 期望: `b_low_cost`
- 实际: `b_high_cost`
  - Run 1 reasoning: `对位slot1已锁water;我slot1出高费wood克water×1.5=15伤,后续slot2-4用低费wood覆盖,预算够。`

### ❌ G_能量预算_对手wood
- 场景: slot1 选 4费爆预算后剩 2 能量给 3slot → 应选 1费克制保节奏
- 期望: `b_low_cost`
- 实际: `b_high_cost`
  - Run 1 reasoning: `对位slot1已锁wood;我slot1出高费fire克wood×1.5=15伤,后续slot2-4用低费fire覆盖,能量够。`

### ❌ H_膨胀能量_R5_base10
- 场景: base_energy=10（R5），玩家锁高威胁fire；候选 3 费 water 强克 / 1 费 water 弱克 / 2 费 fire 中性。LLM 应读 self.base_energy 而非假设 6，并优先选克制色
- 期望: `b_3cost_counter`
- 实际: `b_2cost_neutral`
  - Run 1 reasoning: `玩家预判我出水克火;我偏出fire同色打乱节奏留water给后续slot。`
...
- 期望: `b_heal_big`
- 实际: `b_high_dmg`
  - Run 1 reasoning: `slot4已锁wood;我两张火牌都克wood×1.5,纯伤火10伤>凤凰击+8伤+8治(已满血),选纯伤火最大化伤害。`
